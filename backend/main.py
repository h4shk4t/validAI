...
def rag_request():
    try:
        ...
        hash = upload(prompt)
        print(f"Hash: {hash}")
        # Make a json post request to a backend endpoint with fields 'hash' and 'model_name'
        # The endpoint should return the answer generated by the model
        # The answer should be returned in a json object with the key 'answer'
        answer = requests.post("http://localhost:3000/rag", json={"hash": hash, "model_name": model_name}).json()["answer"] 
